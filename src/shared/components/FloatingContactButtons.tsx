...
export const FloatingContactButtons: React.FC = () => {
  ...
  return (
    <div
      ...
      } ${isRTL ? 'left-3 sm:left-6' : 'right-3 sm:right-6'} z-40 flex flex-col gap-2 items-end rtl:items-start animate-fade-in transition-all duration-300`}
    >
      {/* WhatsApp Button */}
      <a
        href={whatsappUrl}
        ...
        className="group flex items-center justify-center gap-2 p-2.5 sm:px-3.5 sm:py-2.5 bg-[#25D366] text-white rounded-full shadow-lg hover:bg-[#20ba5a] hover:scale-105 active:scale-95 transition-all duration-300"
        title="Contact via WhatsApp / تواصل معنا عبر واتساب"
        aria-label="WhatsApp"
      >
        <WhatsAppIcon className="w-4 h-4 sm:w-5 sm:h-5 fill-current" />
        <span className="hidden sm:inline max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-300 text-xs font-mono font-bold whitespace-nowrap">
          {t.whatsapp}
        </span>
      </a>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="group flex items-center justify-center gap-2 p-2.5 sm:px-3.5 sm:py-2.5 bg-[#1877F2] text-white rounded-full shadow-lg hover:bg-[#166fe5] hover:scale-105 active:scale-95 transition-all duration-300"
