...
export const HeroSection: React.FC = () => {
  ...
  const { homeSettings } = useStoreData();
  const hero = homeSettings?.hero;

  const tag = isRTL ? (hero?.tagAr || t.heroSeason) : (hero?.tagEn || t.heroSeason);
  const title = isRTL ? (hero?.titleAr || t.heroTitle) : (hero?.titleEn || t.heroTitle);
  const subtitle = isRTL ? (hero?.subtitleAr || t.heroSubtitle) : (hero?.subtitleEn || t.heroSubtitle);
  const buttonText = isRTL ? (hero?.buttonTextAr || t.exploreCollection) : (hero?.buttonTextEn || t.exploreCollection);
  const buttonLink = hero?.buttonLink || '/collections/men';
  const secondaryButtonText = isRTL ? (hero?.secondaryButtonTextAr || t.viewLookbook) : (hero?.secondaryButtonTextEn || t.viewLookbook);
  const secondaryButtonLink = hero?.secondaryButtonLink || '/collections/new-arrivals';
  const heroImage = hero?.imageUrl || `${import.meta.env.BASE_URL}images/products/eiffel-outfit-flatlay.jpg`;
